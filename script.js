...
const EMAILJS_TEMPLATE_TIENDA  = "template_gw6pnam"; // correo a Tres Piedras
const EMAILJS_TEMPLATE_CLIENTE = "template_gw6pnam"; // <-- cambia esto por el ID de tu 2a plantilla
...
window.addEventListener("DOMContentLoaded", function() {
  ...
  var gallery = document.getElementById("galleryStrip");
  [IMGS.campo_riego, IMGS.arboles, IMGS.campo_trabajadores].forEach(function(src) {
...
function renderProducts() {
  document.getElementById("productsGrid").innerHTML = products.map(function(p) {
    var base = p.options.choices[0];
    return (
...
function renderModalPrice() {
  var c = currentProduct.options.choices[currentOptIdx];
...
function renderModalOptions() {
  var label   = currentProduct.options.label;
  var choices = currentProduct.options.choices;
...
function addToCartFromModal(goCheckout) {
  ...
  var choice = currentProduct.options.choices[currentOptIdx];
...
function submitShipping() {
  var required = ["nombre","apellido","email","tel","estado","ciudad","dir","cp"];
  var valid = true;
...
function renderPaymentStep(body) {
  var total       = cart.reduce(function(s, i) { return s + i.price; }, 0);
  var isChihuahua = shippingData.estado === "Chihuahua";
...
// PASO 4 — CONFIRMACIÓN + DOS CORREOS
function confirmOrder(method) {
  ...
  // ── CORREO 1: a Tres Piedras ────────────────────────────────
  emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_TIENDA,
    Object.assign({}, params, { to_email: "trespiedrasjmz@gmail.com" })
  ).then(function() {
    console.log("✅ Correo a tienda enviado");
  }).catch(function(err) {
    console.error("❌ Error correo tienda:", err);
  });

  // ── CORREO 2: confirmación al cliente ──────────────────────
  // Necesitas crear una 2a plantilla en EmailJS y poner su ID arriba en EMAILJS_TEMPLATE_CLIENTE
  // La plantilla del cliente puede usar las mismas variables: {{customer_name}}, {{items}}, {{total}}, etc.
  emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_CLIENTE,
    Object.assign({}, params, { to_email: shippingData.email })
  ).then(function() {
    console.log("✅ Correo a cliente enviado");
  }).catch(function(err) {
    console.error("❌ Error correo cliente:", err);
  });
  ...
  body.innerHTML =
    '<div class="success-screen">' +
      ...
      '<p>Gracias <strong>' + shippingData.nombre + '</strong>, hemos recibido tu pedido.<br><br>' +
        'Te enviamos una confirmación a <strong>' + shippingData.email + '</strong> ' +
        'y en las próximas horas nos comunicaremos al <strong>' + shippingData.tel + '</strong> ' +
        'para coordinar los detalles del envío.<br><br>' +
