// ============================================================
//  RENOVA — script.js
//  Reemplaza los 3 valores de abajo con los de tu cuenta EmailJS
// ============================================================
const EMAILJS_PUBLIC_KEY  = "PdScCSG6OgqwmiTdJ";   // Account → General → Public Key
const EMAILJS_SERVICE_ID  = "service_24kefc9";   // Ya tienes este
const EMAILJS_TEMPLATE_ID = "template_2xglbwh";  // Email Templates → tu plantilla

// ── IMÁGENES (se inyectan desde el HTML) ────────────────────
// Las imágenes viven en el objeto global IMGS definido en el HTML.
...
let currentOptIdx = 0;
let checkoutStep  = "cart";   // "cart" | "shipping" | "payment" | "success"
let shippingData  = {};

// ── INIT ─────────────────────────────────────────────────────
window.addEventListener("DOMContentLoaded", () => {
  // Hero con imagen editada del campo
  document.getElementById("heroBg").style.backgroundImage = `url(${IMGS.campo_panorama})`;
...
function renderModalPrice() {
  ...
  document.getElementById("modalPrice").innerHTML =
    `$${c.price.toLocaleString()} MXN <span style="font-size:.9rem;color:var(--gris);font-weight:400">/ ${c.label}</span>`;
}

function renderModalOptions() {
  const { label, choices } = currentProduct.options;
  document.getElementById("modalOptions").innerHTML = `
    <div class="modal-label">${label}</div>
    <div class="options">
      ${choices.map((c, i) => `
        <button class="opt-btn${i === 0 ? " selected" : ""}" onclick="selectOpt(this, ${i})">
...
function selectOpt(btn, idx) {
  document.querySelectorAll("#modalOptions .opt-btn").forEach(b => b.classList.remove("selected"));
...
function addToCartFromModal(goCheckout = false) {
...
function quickAdd(id) {
  ...
  cart.push({ ...p, selectedOpt: c.label, price: c.price });
  updateCartCount();
...
function renderCartStep(body) {
  if (!cart.length) {
    body.innerHTML = `
      <div style="text-align:center;padding:3rem 1rem">
        ...
        <h3 style="font-family:'Playfair Display',serif;font-size:1.5rem;color:var(--tierra);margin-bottom:.8rem">
          Tu carrito está vacío
        </h3>
        <p style="color:var(--gris);font-size:.9rem;margin-bottom:1.5rem">
          Agrega productos desde la tienda
        </p>
        <button
          onclick="closeCheckout();setTimeout(()=>document.getElementById('tienda').scrollIntoView({behavior:'smooth'}),300)"
          style="padding:.8rem 2rem;background:var(--hoja);color:#fff;border:none;border-radius:10px;
                 font-size:.85rem;letter-spacing:.06em;text-transform:uppercase;cursor:pointer;font-weight:600">
          Ver tienda
        </button>
      </div>`;
    return;
  ...
  body.innerHTML = `
    <div class="checkout-section-title">Productos en tu carrito</div>
    ${cart.map((item, idx) => `
      <div class="cart-item">
    ...
    <button class="btn-confirm" onclick="checkoutStep='shipping';renderCheckout()">
      Continuar con el envío &rarr;
    </button>`;
}
...
function renderShippingStep(body) {
  ...
  const opts = states
    .map(s => `<option value="${s}"${s === "Chihuahua" ? " selected" : ""}>${s}</option>`)
    .join("");

  body.innerHTML = `
    <button onclick="checkoutStep='cart';renderCheckout()"
      style="background:none;border:none;color:var(--gris);cursor:pointer;font-size:.82rem;
    ...
    <div class="form-row-2">
      <div class="form-group">
        <label>Nombre *</label>
        <input id="sh_nombre" type="text" placeholder="Tu nombre"/>
      </div>
      <div class="form-group">
        <label>Apellido *</label>
        <input id="sh_apellido" type="text" placeholder="Tu apellido"/>
      </div>
    </div>
    <div class="form-group">
      <label>Email *</label>
      <input id="sh_email" type="email" placeholder="tu@correo.com"/>
    </div>
    <div class="form-group">
      <label>Teléfono *</label>
      <input id="sh_tel" type="tel" placeholder="+52 614 000 0000"/>
    </div>
    <div class="form-group">
      <label>Estado *</label>
      <select id="sh_estado">
    ...
    <div class="form-group">
      <label>Ciudad *</label>
      <input id="sh_ciudad" type="text" placeholder="Tu ciudad"/>
    </div>
    <div class="form-group">
      <label>Dirección *</label>
      <input id="sh_dir" type="text" placeholder="Calle, número, colonia"/>
    </div>
    <div class="form-group">
      <label>Código Postal *</label>
      <input id="sh_cp" type="text" placeholder="33980"/>
    </div>
    <div class="form-group">
      <label>Notas adicionales</label>
      <textarea id="sh_notas" placeholder="Instrucciones especiales, referencias…"></textarea>
    </div>
    <button class="btn-confirm" onclick="submitShipping()">Continuar al pago &rarr;</button>`;

  // Rellenar campos si el usuario regresó
...
function submitShipping() {
  ...
  let valid = true;
  required.forEach(f => {
    const el = document.getElementById("sh_" + f);
    if (!el || !el.value.trim()) {
      valid = false;
  ...
  body.innerHTML = `
    ...
    <div style="background:var(--crema);border-radius:10px;padding:1rem 1.2rem;margin-bottom:1rem;font-size:.85rem">
      <div style="display:flex;justify-content:space-between;margin-bottom:.3rem">
        ...
        <span style="font-weight:600">
          ${shippingData.nombre} ${shippingData.apellido} · ${shippingData.ciudad}, ${shippingData.estado}
        </span>
      </div>
    ...
    <div id="payDetailsTransfer" class="pay-panel">
      <div class="transfer-box">
        ...
        <div class="transfer-row">
          ...
          <span>
            012345678901234567
            <button class="copy-btn" onclick="copyText('012345678901234567','CLABE copiada')">Copiar</button>
          </span>
        </div>
      ...
      <p style="font-size:.8rem;color:var(--gris);margin-bottom:1rem">
        Después de realizar la transferencia, confirma tu pedido y te contactamos para coordinar el envío.
      </p>
    ...
    <div id="payDetailsCard" class="pay-panel">
      <div class="transfer-box">
        ...
        <p style="font-size:.85rem;color:#555;line-height:1.7;margin-bottom:.8rem">
          Escanea el QR con la app de tu banco (BBVA, Santander, Banorte, etc.) para pagar de inmediato.
        </p>
        <div style="text-align:center;padding:1.5rem;background:#fff;border-radius:10px;border:2px dashed var(--hoja)">
          ...
          <p style="font-size:.78rem;color:var(--gris);margin-top:.3rem">
            Monto: <strong>$${total.toLocaleString()} MXN</strong>
          </p>
        </div>
      </div>
      <button class="btn-confirm" onclick="confirmOrder('CoDi / QR Bancario')" style="margin-top:1rem">
        ✓ Confirmar pedido
      </button>
    </div>

    <!-- Panel: Efectivo (solo Chihuahua) -->
    ${isChihuahua ? `
    <div id="payDetailsCash" class="pay-panel">
      <div class="cash-box">
        <h4>💵 Pago en efectivo — Chihuahua</h4>
        <p>
          Recibirás tu pedido y pagarás en el momento de la entrega. Nuestro equipo se comunicará
          contigo para coordinar la fecha y hora en
          <strong>${shippingData.ciudad}, ${shippingData.estado}</strong>.
        </p>
...
function selectPayment(type) {
  ...
  const opt   = document.getElementById("payOpt"     + cap);
  const panel = document.getElementById("payDetails" + cap);
...
function confirmOrder(method) {
  ...
  const itemsList = cart
    .map(i => `${i.name} (${i.selectedOpt}) — $${i.price.toLocaleString()}`)
    .join("\n");
...
function renderSuccessStep(body) {
  body.innerHTML = `
    <div class="success-screen">
      ...
      <p>
        Gracias <strong>${shippingData.nombre}</strong>, hemos recibido tu pedido.<br><br>
...
function sendEmail(params) {
  if (EMAILJS_PUBLIC_KEY === "YOUR_PUBLIC_KEY") {
    console.log("EmailJS no configurado aún. Datos del pedido:", params);
    return;
  }
...
function sendContactForm() {
  ...
  ["c_nombre","c_apellido","c_email","c_tel","c_mensaje"].forEach(id => {
...
window.addEventListener("scroll", () => {
  ...
  const sections = ["inicio", "acerca", "tienda", "contacto"];
  let current    = "inicio";
  sections.forEach(id => {
    const el = document.getElementById(id);
